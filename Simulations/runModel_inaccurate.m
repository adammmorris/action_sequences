%% model
% Simulates model-based agents with the inaccurate belief structure suggested by David Melnikoff.
...
function [results] = runModel_inaccurate(envInfo, params)
...
states_b = envInfo{1};
actions_b = envInfo{2};
transition_probs_b = envInfo{5};

numStates_b = max([states_b{:}]);
numActions_b = max([actions_b{:}]);
...
for thisAgent = 1:numAgents
    ...
    Q_MB_b = zeros(numStates_b, numActions_b);
    ...
    for thisRound=1:numRounds
        ...
        S2s_b = states_b{2};
        S3s_b = states_b{3};
        S1_choices_b = actions_b{S1}; % both actions & sequences
        S1_actions_b = S1_choices_b; % just actions
        ...
        Q_MB_b(S1_b, S1_choices_b) = 0;
        if w_MB > 0
            for a1 = S1_actions_b
                % Loop through subsequent states
                for nextState = S2s_b
                        Q_MB_b(S1_b, a1) = Q_MB_b(S1_b, a1) + transition_probs_b(S1_b, a1, nextState) * ...
                            max(permute(transition_probs_b(nextState, actions_b{nextState}, S3s_b), [2 3 1]) *...
                            Q_MB_b(S3s_b, 1));
                end
            end
        end
        ...
        Q_weighted(S1_actions) = w_MB * Q_MB(S1, S1_actions) + w_MF * Q_MB_b(S1, S1_actions);
        ...
        if any(choice1 == S1_actions)
            ...
            % Where does the bargh model think we are?
            if action1 == 1
                if S2 == 2, S2_b = 2;
                elseif S2 == 3, S2_b = 4;
                end
            ...
            Q_MB_b(S2_b, S2_choices_b) = permute(transition_probs_b(S2_b, S2_choices_b, states_b{3}), [2 3 1]) * Q_MB_b(states_b{3}, 1);
        
            % Combine Q vals
            Q_weighted = w_MB * Q_MB(S2, S2_choices) + w_MF * Q_MB_b(S2_b, S2_choices_b);
...
%                 transition_probs(S2, choice2, :) = transition_probs(S2, choice2, :) / sum(transition_probs(S2, choice2, :));
        ...
        if extremeRep
            ...
            % figure out what stage3 the bargh model thinks we're in
            if action1 == 1
                if S3 == 4, S3_b = 6;
                elseif S3 == 5, S3_b = 7;
                elseif S3 == 6, S3_b = 10;
            ...
            Q_MB_b(S3_b,1) = Q_MB_b(S3_b,1) + lr * (reward_normed - Q_MB_b(S3_b, 1));
        end
