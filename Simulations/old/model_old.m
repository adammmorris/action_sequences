...
for thisAgent = 1:numAgents
    ...
    for thisRound=1:numRounds
        ...
        if any(choice1 == S1_actions)
            ...
            delta = reward_normed - Q_MF(S2_MF, choice2);
        ...
        elseif any(choice1 == S1_seqs)
            ...
            if ~extremeRep
                ...
                transition_probs(S1, choice1, S3) = transition_probs(S1, choice1, S3) + ...
                    lr * (1 - transition_probs(S1, choice1, S3));
                % renormalize
                transition_probs(S1, choice1, :) = transition_probs(S1, choice1, :) / sum(transition_probs(S1, choice1, :));
            end
            
            % Update
            Q_MF(S1, choice1) = Q_MF(S1, choice1) + lr * (reward_normed - Q_MF(S1, choice1));
